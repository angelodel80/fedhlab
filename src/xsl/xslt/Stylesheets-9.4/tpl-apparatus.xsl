  ...
  <xsl:template match="*" mode="app-word">
   ...
   <xsl:template name="title-convert">
      ...
      <xsl:variable name="no-l">
         <xsl:choose>
            <xsl:when test="$apptype = 'siccorr'"><xsl:value-of select="substring-before(replace($scribeswitch, 'l\. ', 'for which correct to '), '(corr)')"/></xsl:when>
            ...
            <xsl:otherwise><xsl:value-of select="replace($scribeswitch, 'l\. ', 'Scribe wrote ')"/></xsl:otherwise>
         </xsl:choose>
      </xsl:variable>
